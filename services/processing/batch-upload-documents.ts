...
const CLAIMS_RECORDS_FILE_PATH = process.env.CLAIMS_RECORDS_FILE_PATH || "./data/claims-records.json";
...
export async function batchUploadDocuments(batchSize: number): Promise<void> {
    ...
    const allClaims = JSON.parse(await Bun.file(CLAIMS_RECORDS_FILE_PATH).text()) as ClaimRecord[];
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
        ...
        const currentClaims = JSON.parse(await Bun.file(CLAIMS_RECORDS_FILE_PATH).text()) as ClaimRecord[];
        ...
        await Bun.write(CLAIMS_RECORDS_FILE_PATH, JSON.stringify(currentClaims, null, 2));
        console.log(`✓ Saved batch ${batchIndex + 1}/${totalBatches} to ${CLAIMS_RECORDS_FILE_PATH}`);
    }
}