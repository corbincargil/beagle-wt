...
- The decision summary should be detailed and professional.`;
  ...
  try {
    ...
    const jsonText = responseText.replace(/^```json\s*|\s*```$/g, "").replace(/^```\s*|\s*```$/g, "");
    
    const chargesAnalysis = JSON.parse(jsonText) as {
      approvedCharges: Array<{ description: string; amount: number; category?: string }>;
      excludedCharges: Array<{ description: string; amount: number; category?: string }>;
    ...
    const finalPayout = Math.min(approvedChargesTotal, initialResult.maxBenefit);
    ...
    if (error instanceof Error) {
      throw new Error(`Error analyzing charges for claim ${claim.trackingNumber}: ${error.message}`);
    }
    throw error;
  }
}
