 ...
 */
export async function analyzeClaimInitial(claim: ClaimRecord): Promise<InitialClaimResult> {
  if (!CLAUDE_API_KEY) {
...
Important: Return ONLY valid JSON, no markdown formatting or code blocks.`;
  ...
  try {
    ...
    if (error instanceof Error) {
      throw new Error(`Error analyzing claim ${claim.trackingNumber}: ${error.message}`);
    }
    throw error;
  }
}
